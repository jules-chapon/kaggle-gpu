...
def get_parser(
    ...
    parser.add_argument(
        ...
        "--output-dir",
        type=str,
        default=ROOT_DIR / OUTPUT_FOLDER_NAME,
        help="Output directory",
    )
    parser.add_argument(
        "-nowb", "--no-wandb", action="store_true", help="Disable weights and biases"
    )
...
def training_loop(
    ...
    for n_epoch in tqdm(range(config[NB_EPOCHS])):
        current_loss = {TRAIN: 0.0, VALIDATION: 0.0, TEST: 0.0}
...
def train(config: dict, output_dir: Path, device: str = "cuda"):
