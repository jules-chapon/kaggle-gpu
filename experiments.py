...
def get_experiment_config(exp: int) -> dict:
    config = {ID: exp, NAME: f"{exp:04d}", NB_EPOCHS: 5}
    config[DATALOADER] = {BATCH_SIZE: {TRAIN: 32, VALIDATION: 32, TEST: 32}}
    config[OPTIMIZER] = {NAME: "Adam", PARAMS: {LR: 1e-3}}
    if exp == 0:
        config[MODEL] = {
            ARCHITECTURE: dict(conv_feats=8, kernel_size=3),
            NAME: "ConvModel",
        }
    elif exp == 1 or exp == 2:
        config[MODEL] = {
            ARCHITECTURE: dict(conv_feats=16, kernel_size=5),
            NAME: "ConvModel",
        }
        if exp == 2:
...
) -> Tuple[ConvModel, torch.optim.Optimizer, dict]:
